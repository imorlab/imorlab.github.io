...
const props = defineProps({
  variant: {
    type: String,
    default: 'square',
    validator: (v) => ['square', 'circle', 'triangle', 'diamond'].includes(v)
  },
  pixelSize: { type: Number, default: 3 },
  color: { type: String, default: '#B19EEF' },
...
const cleanup = () => {
  ...
  t.composer?.dispose();
  t.renderer.dispose();
  if (t.renderer.domElement.parentElement === containerRef.value) {

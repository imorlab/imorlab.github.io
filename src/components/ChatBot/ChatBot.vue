        ...
        >
          ...
          >
            <div
              ...
              v-html="message.html"
            ></div>
            <div v-else class="text-sm leading-relaxed whitespace-pre-wrap">{{ message.content }}</div>
...
// El servicio (base de conocimiento, ~35 kB) se carga solo cuando hace falta:
// no entra en el bundle inicial ni se ejecuta durante el SSG.
let servicePromise = null
const loadService = () => {
  if (!servicePromise) {
    servicePromise = import('./chatbotService.js').then((m) => m.chatbotService)
  }
  return servicePromise
}

// ---- Mensaje de ayuda flotante ----
// Todos los timers se registran para poder limpiarlos al desmontar.
...
const MAX_HELP_CYCLES = 3 // deja de insistir tras unas pocas veces
let helpCycles = 0

const startHelpMessageTimer = () => {
  clearTimers()
  if (helpCycles >= MAX_HELP_CYCLES) return
  after(5000, () => {
    ...
    after(3000, () => {
      ...
      after(15000, startHelpMessageTimer)
    })
  })
}
...
const QUICK_QUESTIONS = {
  experience: { es: '¿Cuál es la experiencia profesional de Israel?', en: "What is Israel's professional experience?" },
  skills: { es: '¿Qué habilidades técnicas tiene Israel?', en: 'What technical skills does Israel have?' },
  projects: { es: '¿Cuáles son los proyectos más destacados de Israel?', en: "What are Israel's most notable projects?" },
  contact: { es: '¿Cómo puedo contactar con Israel?', en: 'How can I contact Israel?' }
}
...
const pushBotMessage = (content) => {
  messages.value.push({
    ...
    type: 'bot',
    content,
    html: processMarkdown(content), // se calcula una vez, no en cada render
    timestamp: new Date()
  })
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    const response = await service.getResponse(text, locale.value)

    // Pequeña pausa para simular escritura
    await new Promise((resolve) => after(600 + Math.random() * 600, resolve))
...
const formatTime = (timestamp) =>
  timestamp.toLocaleTimeString(locale.value === 'es' ? 'es-ES' : 'en-US', { hour: '2-digit', minute: '2-digit' })

// Markdown mínimo → HTML. Solo se aplica a respuestas del bot (contenido propio, no del usuario).
...
// Si cambia el idioma, la conversación anterior deja de tener sentido: se reinicia.
watch(locale, clearConversation)

// Los timers solo en cliente (nunca durante el SSG)
onMounted(startHelpMessageTimer)
